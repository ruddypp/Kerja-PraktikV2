    ...
    const updatedMaintenance = await prisma.$transaction(async (prisma) => {
      ...
      await prisma.maintenanceStatusLog.create({
        data: {
          ...
          status: RequestStatus.COMPLETED,
          userId: user.id,
          notes: `Maintenance selesai. CSR No: ${csrNumber}, TCR No: ${tcrNumber}`,
        },
      });
      ...
      for (const historyItem of historyItems) {
        await prisma.itemHistory.update({
          ...
          data: {
            endDate: new Date(),
            details: `Maintenance selesai: ${serviceReportData?.findings || "Maintenance selesai"}. CSR No: ${csrNumber}, TCR No: ${tcrNumber}`,
          },
        });
      }
      ...
      await logMaintenanceActivity(
        ...
        `Maintenance selesai untuk ${maintenance.item.name} (${maintenance.itemSerial}). CSR No: ${csrNumber}, TCR No: ${tcrNumber}`
      );
