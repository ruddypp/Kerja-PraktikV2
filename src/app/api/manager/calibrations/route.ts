...
export async function POST(request: Request) {
  try {
    ...
    await prisma.calibrationStatusLog.create({
        ...
        calibrationId: id,
        status: COMPLETED as RequestStatus,
        notes: notes || 'Calibration approved by Manager',
        userId: user.id
      }
    });
...
export async function PATCH(request: Request) {
  try {
    ...
    if (status === COMPLETED && String(calibration.status) !== COMPLETED) {
      ...
      if (validUntil) {
        ...
        if (reminderDate > new Date()) {
          // Get an manager user for notifications
          const managerUser = await prisma.user.findFirst({
            where: { role: 'MANAGER' }
          });
          
          if (!managerUser) {
            console.error('No manager user found for sending notification');
          } else {
            ...
            // Create reminder notification for manager
            await prisma.notification.create({
              data: {
    ...
    if (status === COMPLETED && validUntil && managerUser) {
      const validUntilDate = new Date(validUntil);
      const reminderDate = new Date(validUntilDate);
      reminderDate.setDate(reminderDate.getDate() - 30);
