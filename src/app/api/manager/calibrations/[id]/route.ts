...
export async function PATCH(
  ...
  try {
    ...
    console.log('Allowing update to calibration. User ID:', user.id, 'Calibration ID:', calibration.id, 'Is manager:', isManager);
    
    // Prepare update data with different permissions for manager vs owner
    ...
    // Create status log if status changed (manager only)
    if (isManager && status) {
...
export async function DELETE(
  ...
  try {
    // Verifikasi user manager menggunakan helper function yang konsisten
    const user = await getUserFromRequest(request);
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      try {
        ...
        await tx.itemHistory.updateMany({
          ...
          data: {
            details: `Calibration record deleted by manager (${user.name})`,
            endDate: new Date()
          }
        });
