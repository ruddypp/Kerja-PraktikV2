...
// Add POST method to allow manager to create maintenance
export async function POST(req: NextRequest) {
  try {
    ...
    const maintenance = await prisma.$transaction(async (prisma) => {
      ...
      await prisma.maintenanceStatusLog.create({
        data: {
          ...
          status: RequestStatus.PENDING,
          userId: user.id,
          notes: "Maintenance dimulai oleh manager",
        },
      });
      ...
      await prisma.itemHistory.create({
        data: {
          itemSerial,
          action: "MAINTAINED",
          details: "Barang mulai dalam proses maintenance (dibuat oleh manager)",
    ...
    await logMaintenanceActivity(
      ...
      maintenance.id,
      itemSerial,
      `Manager memulai maintenance untuk barang ${itemSerial}`
    );
