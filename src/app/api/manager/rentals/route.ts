...
// POST - Create a new rental request by manager
export async function POST(req: NextRequest) {
  try {
    ...
    const rental = await prisma.$transaction(async (tx) => {
      // Create a new rental (APPROVED by default when manager creates it)
      const newRental = await tx.rental.create({
        data: {
          itemSerial,
          userId: userId, // Use the target user or manager as fallback
          status: RequestStatus.APPROVED, // Auto-approve manager rentals
      ...
      await tx.rentalStatusLog.create({
        data: {
          ...
          userId: user.id, // Manager is the one changing the status
          notes: 'Rental created and approved by manager'
        }
      });
      ...
      // Create notification for the user if it's not the manager
      if (targetUserId && targetUserId !== user.id) {
        await tx.notification.create({
          data: {
            userId: targetUserId,
            title: 'New Rental Created',
            message: `Manager has created a rental for ${item.name} on your behalf`,
            type: NotificationType.RENTAL_STATUS_CHANGE,
