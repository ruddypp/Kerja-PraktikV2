    ...
    const updatedMaintenance = await prisma.$transaction(async (prisma) => {
      ...
      await prisma.maintenanceStatusLog.create({
        data: {
          ...
          status: RequestStatus.COMPLETED,
          userId: user.id,
          notes: `Maintenance selesai (oleh admin). CSR No: ${csrNumber}, TCR No: ${tcrNumber}`,
        },
      });
      ...
      for (const historyItem of historyItems) {
        await prisma.itemHistory.update({
          ...
          data: {
            endDate: new Date(),
            details: `Maintenance selesai: ${serviceReportData?.findings || "Maintenance selesai"} (oleh admin). CSR No: ${csrNumber}, TCR No: ${tcrNumber}`,
          },
        });
      }
