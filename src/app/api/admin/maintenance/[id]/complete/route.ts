...
export async function POST(
  ...
  try {
    ...
    console.log("Received data for maintenance completion by admin:");
    ...
    const updatedMaintenance = await prisma.$transaction(async (prisma) => {
      ...
      await prisma.maintenanceStatusLog.create({
        data: {
          ...
          status: RequestStatus.COMPLETED,
          userId: user.id,
          notes: "Maintenance selesai (oleh admin)",
        },
      });
      ...
      for (const historyItem of historyItems) {
        await prisma.itemHistory.update({
          ...
          data: {
            endDate: new Date(),
            details: `Maintenance selesai: ${serviceReportData?.findings || "Maintenance selesai"} (oleh admin)`,
          },
        });
      }
      ...
      await logMaintenanceActivity(
        ...
        `Maintenance selesai untuk ${maintenance.item.name} (${maintenance.itemSerial}) oleh admin`
      );
