...
const CACHE_KEY_PREFIX = 'manager_maintenance_items_';
const SEARCH_CACHE_PREFIX = 'manager_maintenance_search_';
...
export default function ManagerNewMaintenancePage() {
  ...
  const fetchItems = useCallback(async (page: number, limit: number, searchQuery: string = '') => {
    try {
      ...
      const response = await fetch(`/api/manager/items?${params.toString()}`);
  ...
  const searchItems = useCallback(async (term: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/manager/items?${params.toString()}`, {
