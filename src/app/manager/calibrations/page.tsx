...
export default function ManagerCalibrationsPage() {
  ...
  useSWR('/api/manager/vendors', fetcher, {
    onSuccess: (data) => {
  ...
  const fetchAvailableItems = async () => {
    try {
      ...
      const res = await fetch(`/api/manager/items?status=AVAILABLE&timestamp=${new Date().getTime()}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <DashboardLayout>
      <div className="container mx-auto px-4 py-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <Link href="/manager/vendors" className="btn btn-secondary">
            Manage Vendors
          </Link>
          </div>
        </div>
