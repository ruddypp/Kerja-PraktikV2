...
export default function ManagerCalibrationsPage() {
  ...
  const resetCompleteForm = () => {
    setCompleteForm({
      ...
      approvedBy: '',
      validUntil: format(new Date(Date.now() + 365*24*60*60*1000), 'yyyy-MM-dd'),
      certificateNumber: '', // Keep in state but not used in API calls
      calibrationDate: format(new Date(), 'yyyy-MM-dd'),
      notes: ''
    });
  };
  ...
  useSWR('/api/manager/vendors?limit=10000', fetcher, {
    onSuccess: (data) => {
  ...
  useSWR('/api/manager/statuses?type=calibration', fetcher, {
  ...
  const { mutate } = 
    useSWR(() => `/api/manager/calibrations?statusId=${filter.statusId}&vendorId=${filter.vendorId}&page=${filter.page}&limit=${PAGE_SIZE}`, 
    fetcher, {
  ...
  const openCompleteModal = (calibration: Calibration) => {
    ...
    setCompleteForm({
      ...
      // Certificate Number is auto-generated, but keep in state for form consistency
      certificateNumber: '',
  ...
  const handleUpdateCalibration = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/manager/calibrations/${selectedCalibration.id}`, {
        method: 'PATCH',
            headers: {
  ...
  const getStatusBadgeColor = (statusName: string | undefined) => {
    if (!statusName) {
      return 'bg-gray-100 text-gray-800'; // Default style for undefined status
    }
  ...
  const getDisplayStatus = (status: string | { name: string } | undefined): string => {
    if (!status) {
      return 'PENDING'; // Default status if undefined
    }
  ...
  const handleComplete = async (id: string) => {
    try {
      ...
      const response = await fetch(`/api/manager/calibrations/${id}/complete`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const openEditCertificateModal = async (calibration: Calibration) => {
    try {
      ...
      const response = await fetch(`/api/manager/calibrations/${calibration.id}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleUpdateCertificate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/manager/calibrations/${selectedCalibration.id}/certificate`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const fetchAvailableItems = async () => {
    try {
      ...
      const res = await fetch(`/api/manager/items?status=AVAILABLE&limit=20000&timestamp=${new Date().getTime()}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link href="/manager/vendors" className="bg-gray-600 hover:bg-gray-700 text-white px-3 py-2 rounded-md flex items-center justify-center text-sm">
            Manage Vendors
          </Link>
          </div>
        </div>
                      ...
                      {calibration.validUntil ? formatDate(calibration.validUntil) : 'N/A'}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          ...
                          {/* Download certificate */}
                              <a 
                                href={`/api/manager/calibrations/${calibration.id}/certificate`} 
                                target="_blank" 
