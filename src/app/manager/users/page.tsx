...
export default function ManagerUsersPage() {
  ...
  const CACHE_KEY = 'manager_users_data';
  const CACHE_TIMESTAMP_KEY = 'manager_users_last_fetch';
  ...
  const fetchData = useCallback(async (searchTerm = search) => {
    try {
      ...
      const usersRes = await fetch(`/api/manager/users${queryString}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let apiUrl = '/api/manager/users';
      let method = 'POST';
      let successMessage = 'User created successfully';
      
      // If editing, use PUT and include ID
      if (isEditMode && currentUser) {
        apiUrl = `/api/manager/users/${currentUser.id}`;
        method = 'PUT';
