...
export default function InventoryExecutionPage() {
    ...
    const fetchOrCreateExecution = async () => {
      try {
        ...
        if (executionId) {
          ...
          const res = await fetch(`/api/manager/inventory-schedules/execution/${executionId}`);
          ...
          const res = await fetch('/api/manager/inventory-schedules/execution', {
            method: 'POST',
            headers: {
          ...
          router.replace(`/manager/inventory/schedules/execution?scheduleId=${scheduleId}&executionId=${data.id}`);
        } else {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await fetch(`/api/manager/inventory-schedules/execution/${execution?.id}/complete`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      setTimeout(() => {
        router.push('/manager/inventory/schedules');
      }, 3000);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'An error occurred');
