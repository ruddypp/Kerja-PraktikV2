...
export default function ActivityLogsPage() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Continue without user filtering if this fails
    }
  };
  ...
  const generatePDF = () => {
    try {
      ...
      doc.text(`Generated on: ${format(new Date(), 'dd MMM yyyy HH:mm')}`, 14, yPos);
      yPos += 5;
      
      if (filters.userId) {
        const user = users.find(u => u.id.toString() === filters.userId);
        doc.text(`User: ${user ? user.name : filters.userId}`, 14, yPos);
        yPos += 5;
      }
      ...
      // Table data
      const tableColumn = ['#', 'Date & Time', 'User', 'Activity'];
      const tableRows = activityLogs.map((log, index) => [
        (index + 1).toString(),
        formatDate(log.createdAt),
        log.user ? log.user.name : `User ID: ${log.userId}`,
        log.activity
      ]);
    ...
    <DashboardLayout>
      <div>
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-title text-xl md:text-2xl">Activity Logs</h1>
          <button
            onClick={generatePDF}
        ...
        <div className="card mb-6 border border-gray-200">
          <h2 className="text-subtitle mb-4">Filter Activity Logs</h2>
        ...
        {loading ? (
          <div className="text-center py-8">
            ...
            <p className="mt-4 text-subtitle">Loading activity logs...</p>
          </div>
        ) : activityLogs.length === 0 ? (
          <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded shadow-sm">
            <p className="text-yellow-700 font-medium">No activity logs found. Try adjusting your filters.</p>
          </div>
        ) : (
                  ...
                  {activityLogs.map((log, index) => (
                    <tr key={log.id} className="hover:bg-gray-50">
                      ...
                      <td className="table-cell">
                        <div className="text-subtitle">{log.user?.name}</div>
                        <div className="text-muted text-xs">{log.user?.role.name}</div>
                      </td>
