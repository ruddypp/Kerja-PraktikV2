...
const CACHE_KEY = 'manager_dashboard_data';
const CACHE_TIMESTAMP_KEY = 'manager_dashboard_timestamp';
const SEARCH_CACHE_PREFIX = 'manager_dashboard_search_';
...
export default function ManagerDashboard() {
  ...
  const searchItems = useCallback(async (term: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/manager/items?search=${encodeURIComponent(term)}&limit=5`, {
  ...
  const handleItemSelect = (item: InventoryItem) => {
    router.push(`/manager/inventory/history/${encodeURIComponent(item.serialNumber)}`);
    setSearchTerm('');
    setShowSuggestions(false);
  };
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
              <div className="flex justify-between items-center mb-5">
                ...
                <Link href="/manager/notifications" className="text-sm text-blue-600 hover:text-blue-700 hover:underline inline-flex items-center">
                  View all
